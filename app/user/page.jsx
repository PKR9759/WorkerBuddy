...
import { 
  MapPin, Search, Filter, Star, Clock, Briefcase, Zap, 
  Shield, Award, Calendar, ChevronDown, X, SlidersHorizontal,
  Phone, Mail, MapIcon, Users, CheckCircle
} from "lucide-react";
...
export default function UserHomePage() {
  ...
  const skillCategories = [
    ...
    "Cleaner", "Gardener", "Chef", "Driver", "Security", 
    "Technician", "Mason", "Welder", "AC Repair", "Appliance Repair"
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 text-white px-4 py-8">
      ...
      {loading ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-lg">Finding the best workers for you...</p>
        </div>
      ) : (
          ...
          {workers.length > 0 && (
            <div className="mb-6 flex items-center justify-between">
              <p className="text-gray-400">
                Found <span className="text-blue-400 font-semibold">{workers.length}</span> worker{workers.length !== 1 ? 's' : ''} 
                {filters.location && <span> in <span className="text-white">{filters.location}</span></span>}
              </p>
              <div className="text-sm text-gray-500">
                Sorted by {sortOptions.find(opt => opt.value === filters.sortBy)?.label.replace(/^[^\s]+\s/, '')}
              </div>
            </div>
          )}
           ...
           >
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {worker.skills.slice(0, 2).map((skill, index) => (
                          <span key={index} className="inline-flex items-center px-2 py-1 bg-blue-500/20 border border-blue-500/30 text-blue-400 text-xs rounded-full">
                            {skill}
                          </span>
                        ))}
                        {worker.skills.length > 2 && (
                          <span className="text-xs text-gray-400">+{worker.skills.length - 2} more</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-gray-500 text-sm">({worker.reviewCount || 0})</span>
                    </div>
                    <div className="flex items-center gap-1 text-gray-400">
                      <Users className="w-4 h-4" />
                      <span className="text-sm">{worker.completedJobs || 0} jobs</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-medium text-gray-300">Today's Schedule:</h3>
                  </div>
                  ...
                  {isWorkerAvailableToday(worker) ? (
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="text-gray-500 text-sm">Not available today</span>
                    )}
                  </div>
                  ...
                  <div className="text-center">
                    <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${
                      ...
                      <div className={`w-2 h-2 rounded-full ${
                        isWorkerAvailableToday(worker) ? 'bg-emerald-400 animate-pulse' : 'bg-gray-400'
                      }`}></div>
                      {isWorkerAvailableToday(worker) ? 'Available Today' : 'Not Available Today'}
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className="text-center py-16">
                <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-700/30 p-12 max-w-md mx-auto">
                  ...
                  <p className="text-gray-400 mb-6">
                    No workers match your current search criteria. Try adjusting your filters or search terms.
                  </p>
                  <button
                    onClick={clearFilters}
