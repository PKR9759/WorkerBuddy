...
export default function WorkerHome() {
  ...
  const updateBookingStatus = async (bookingId, newStatus) => {
    ...
    try {
      const token = getToken();
      await axios.patch(`/api/worker/bookings/${bookingId}`, 
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 text-white px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-lg">Manage your service requests and appointments</p>
        </div>
        ...
        {bookings.length === 0 ? (
          <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-700/30 p-12 text-center">
            ...
            <h3 className="text-2xl font-semibold mb-4 text-white">No Bookings Available</h3>
            <p className="text-gray-400 text-lg mb-2">No booking requests at the moment.</p>
            <p className="text-gray-500">New booking requests will appear here when customers book your services.</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                      ...
                      <div className={`px-4 py-2 rounded-xl border flex items-center gap-2 ${getStatusColor(booking.status)}`}>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-3 p-3 bg-gray-700/30 border border-gray-600/30 rounded-xl">
                          ...
                          <div>
                            ...
                            <p className="text-white font-medium">{formatDate(booking.scheduledTime)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-gray-700/30 border border-gray-600/30 rounded-xl">
                          ...
                          <div>
                            ...
                            <p className="text-white font-medium">{formatTime(booking.scheduledTime)}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="p-4 bg-gray-700/30 border border-gray-600/30 rounded-xl">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-300 leading-relaxed">{booking.jobDescription}</p>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="lg:min-w-48">
                    ...
                    {booking.status === "accepted" && (
                      <button
                        ...
                        disabled={updatingBooking === booking._id}
                        className="w-full px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 rounded-xl text-white font-medium transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:transform-none flex items-center justify-center gap-2"
                      >
                        {updatingBooking === booking._id ? (
                          <>
                            <Loader2 className="w-5 h-5 animate-spin" />
                    ...
                    {booking.status === "completed" && (
                      <div className="text-center">
                        ...
                        <p className="text-gray-400 text-sm mt-1">Well done!</p>
                      </div>
                    )}
                  </div>
                </div>
